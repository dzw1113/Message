 ...
 * 调整了默认为开发环境
 * @author lenovo
 * apns_production:True 表示推送生产环境，False 表示要推送开发环境； 如果不指定则为推送生产环境
 * YW
 */
public class Options implements PushModel {
    ...
    private boolean apnsProduction = Boolean.parseBoolean(PropertiesUtil.getInstance(
			"/message.properties").getValue(BasePropertyID.PUSH_APNS_PRODUCTION));
