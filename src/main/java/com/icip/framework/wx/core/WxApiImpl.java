package com.icip.framework.wx.core;
...
import com.icip.framework.wx.commons.WxMsgType;
import com.icip.framework.wx.exception.WxRespException;
import com.icip.framework.wx.map.AesException;
import com.icip.framework.wx.map.SHA1;
import com.icip.framework.wx.util.JsonMsgBuilder;
import com.icip.framework.wx.util.SimpleHttpReq;
import com.icip.framework.wx.vo.Article2;
import com.icip.framework.wx.vo.FollowList;
import com.icip.framework.wx.vo.Follower;
import com.icip.framework.wx.vo.Group;
import com.icip.framework.wx.vo.MPAct;
import com.icip.framework.wx.vo.Menu;
import com.icip.framework.wx.vo.OutPutMsg;
import com.icip.framework.wx.vo.Template;
...
public class WxApiImpl implements WxApi {
	...
	private static final Logger log = Logger.getLogger(WxApiImpl.class);
	...
	public void refreshAccessToken() throws WxRespException {
		String url = String.format(WxApiUrl.ACCESS_TOKEN_API, mpAct.getAppId(),
				mpAct.getAppSecret());
		String result = "";
		try {
	...
	public List<String> getServerIp() throws WxRespException {
		...
		List<String> ips = JSONObject.parseArray(tmp.getString("ip_list"),
				String.class);
	...
	public List<Menu> getMenu() throws WxRespException {
		String url = String.format(WxApiUrl.CUSTOM_MENU_API, "get",
				getAccessToken());
		String result = "";
		try {
			result = SimpleHttpReq.get(url);
	...
	public boolean createMenu(Menu... menus) throws WxRespException {
		...
		// 提交并创建菜单
		String url = String.format(WxApiUrl.CUSTOM_MENU_API, "create",
				getAccessToken());
		String result = "";
		try {
	...
	public boolean deleteMenu() throws WxRespException {
		String url = String.format(WxApiUrl.CUSTOM_MENU_API, "delete",
				getAccessToken());
		String result = "";
		try {
			result = SimpleHttpReq.get(url);
	...
	public int creatGroup(String name) throws WxRespException {
		String url = String.format(WxApiUrl.GROUPS_API, "create",
				getAccessToken());
	...
	public List<Group> getGroups() throws WxRespException {
		String url = String
				.format(WxApiUrl.GROUPS_API, "get", getAccessToken());
		String result = "";
		try {
			result = SimpleHttpReq.get(url);
		} catch (IOException e) {
	...
	public boolean renGroup(int id, String name) throws WxRespException {
		String url = String.format(WxApiUrl.GROUPS_API, "update",
				getAccessToken());
	...
	public int getGroupId(String openId) throws WxRespException {
		String url = String.format(WxApiUrl.GROUPS_API, "getid",
				getAccessToken());
	...
	public FollowList getFollowerList(String nextOpenId) throws WxRespException {
		String url = String.format(WxApiUrl.FOLLOWS_USER_API, getAccessToken(),
				nextOpenId);
		String result = "";
		try {
			result = SimpleHttpReq.get(url);
		...
		String openid = JSON.parseObject(result).getJSONObject("data")
				.getString("openid");
	...
	public Follower getFollower(String openId, String lang)
			throws WxRespException {
		String url = String.format(WxApiUrl.USER_INFO_API, getAccessToken(),
				openId, lang);
		String result = "";
		try {
			result = SimpleHttpReq.get(url);
	...
	public boolean sendCustomerMsg(OutPutMsg msg) throws WxRespException {
		String url = String.format(WxApiUrl.CUSTOM_MESSAGE_API,
				getAccessToken());
		String custom_msg = "";
	...
	public boolean sendTemplateMsg(String openId, String templateId,
			...
			throws WxRespException {
		String api_url = String.format(WxApiUrl.TEMPLATE_MESSAGE_API,
				getAccessToken());
		String template_msg = JsonMsgBuilder.create()
				.template(openId, templateId, topColor, url, templates).build();
		String result = "";
		try {
	...
	public String upMedia(String mediaType, File file) throws WxRespException {
		String url = String.format(WxApiUrl.MEDIA_UP_API, mediaType,
				getAccessToken());
		String result = "";
		try {
	...
	public void dlMedia(String mediaId, File file) throws WxRespException {
		...
		String url = String.format(WxApiUrl.MEDIA_DL_API, getAccessToken(),
				mediaId);
		// 检查文件夹是否存在
		if (!file.exists()) {
	...
	public String[] upNews(Article2... articles2s) throws WxRespException {
		...
		String upnews_msg = JsonMsgBuilder.create().uploadNews(articles2s)
				.build();
		String result = "";
		try {
			result = SimpleHttpReq.post(url, SimpleHttpReq.APPLICATION_JSON,
		...
		JSONObject tmp = JSON.parseObject(result);
		String[] results = { tmp.getString("type"), tmp.getString("media_id"),
				tmp.getString("created_at") };
		return results;
	}

	@Override
	public String[] upVideo(String mediaId, String title, String description)
			throws WxRespException {
		String url = String
				.format(WxApiUrl.MEDIA_UPVIDEO_API, getAccessToken());
		String result = "";
		String upvideo_msg = JsonMsgBuilder.create()
				.uploadVideo(mediaId, title, description).build();
		try {
			result = SimpleHttpReq.post(url, SimpleHttpReq.APPLICATION_JSON,
					upvideo_msg);
		} catch (IOException e) {
	...
	public String sendAll(OutPutMsg msg) throws WxRespException {
		...
		String url = "";
		if (to_users.isEmpty()) {
			url = String.format(WxApiUrl.GROUP_NEWS_MESSAGE_API, "sendall",
					getAccessToken());
		} else {
			url = String.format(WxApiUrl.GROUP_NEWS_MESSAGE_API, "send",
					getAccessToken());
		}
	...
	public boolean delSendAll(String msgId) throws WxRespException {
		String url = String.format(WxApiUrl.GROUP_NEWS_MESSAGE_API, "delete",
				getAccessToken());
		String result = "";
	...
	public Map<String, Object> getJsAPISign(String url) throws WxRespException {
		...
		String sign = null;
		try {
			sign = SHA1.calculate(String.format(sign_param, ticket, nonce,
					time, url));
